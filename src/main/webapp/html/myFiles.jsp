...
<script type="text/javascript">
	$(document).ready(function() {
		$.ajax({
			type : "Post",
			//方法所在页面和方法名      
			url : "../files/showFilesOfEmployee",
			contentType : "application/json; charset=utf-8",
			dataType : "json",
			success : function(data) {
				for(var i=0;i<data.length;i++){
					$('#filesTable').append('<tr><td class="usertablerow2" width="20%">'+data[i].filename+'</td><td class="usertablerow2" width="20%">'+data[i].uploadtime+'</td><td class="usertablerow2" width="10%">'+data[i].size+'</td><td class="usertablerow2" width="20%">'+data[i].departments+'</td><td class="usertablerow2" width="20%">'+data[i].employees+'</td><td class="usertablerow2" width="20%"><a href="../html/alterFiles.jsp?department='+data[i].departments+'&employee='+data[i].employees+'&id='+data[i].id+'">修改</a> <a href="../files/downloadFile?id='+data[i].id+'">下载</a> <a href="../files/delete?id='+data[i].id+'">删除</a></td></tr>');
				}
			},
			error : function(err) {
				alert(err);
